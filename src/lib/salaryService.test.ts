import { createSalaryEntry, listSalaries, getSalaryStats, ServiceError } from './salaryService';
...
describe('salaryService', () => {
  ...
  describe('createSalaryEntry', () => {
    it('throws 400 when data is invalid', async () => {
      await expect(createSalaryEntry({}, 'ip')).rejects.toMatchObject({ status: 400 });
    });

    it('throws 429 when rate limited', async () => {
      (isRateLimited as jest.Mock).mockResolvedValue(true);
      await expect(createSalaryEntry({}, 'ip')).rejects.toMatchObject({ status: 429 });
    });

    it('creates entry when valid', async () => {
      (isRateLimited as jest.Mock).mockResolvedValue(false);
      (prisma.salaryEntry.create as jest.Mock).mockResolvedValue({ id: '1', country: 'AR' });
  ...
  describe('listSalaries', () => {
    it('returns paginated results', async () => {
      ...
      (prisma.salaryEntry.findMany as jest.Mock).mockResolvedValue([{ id: '3' }, { id: '2' }]);
    ...
    it('throws 400 for invalid parameters', async () => {
      await expect(listSalaries({ page: 'foo', pageSize: 'bar' })).rejects.toMatchObject({ status: 400 });
    });
  });
  ...
  describe('getSalaryStats', () => {
    ...
    it('calculates stats', async () => {
      (prisma.salaryEntry.findMany as jest.Mock).mockResolvedValue([{ amount: 50 }, { amount: 100 }]);
